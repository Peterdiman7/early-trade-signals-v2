		...
		<div class="detail-page">
			...
			<div class="detail-body">
				<div class="price-hero">
					<div class="ph-main">
						<div class="ph-price">${{ signal.price.toLocaleString('en-US', { minimumFractionDigits: 2 }) }}</div>
						<div :class="['ph-change', signal.change >= 0 ? 'up' : 'down']">{{ signal.change >= 0 ? '+' : '' }}{{ signal.change }}%</div>
					</div>
					<div class="ph-since">
						{{ t('detail.since') }} {{ signal.signalDate }} (${{ signal.signalPrice }}):
						<strong :style="{ color: sigUp ? 'var(--g)' : 'var(--r)' }">{{ sigUp ? '+' : '' }}{{ sigChg }}%</strong>
					</div>
				</div>

				<div class="d-tabs">
					<button :class="['d-tab', { on: dTab === 'overview' }]" @click="dTab = 'overview'">{{ t('detail.tabOverview') }}</button>
					<button :class="['d-tab', { on: dTab === 'analysis' }]" @click="dTab = 'analysis'">{{ t('detail.tabAnalysis') }}</button>
					<button :class="['d-tab', { on: dTab === 'backtrack' }]" @click="dTab = 'backtrack'">{{ t('detail.tabBacktrack') }}</button>
				</div>
				...
				<div v-if="dTab === 'overview'">
					<div class="info-grid">
						...
						<div class="i-cell"><label>{{ t('detail.strength') }}</label>
							<div class="v" :style="{ color: signal.strength === 'Strong' ? 'var(--g)' : signal.strength === 'Moderate' ? 'var(--y)' : 'var(--r)' }">
								{{ strengthLabel }}
							</div>
						</div>
						<div class="i-cell"><label>{{ t('detail.quality') }}</label>
							<div class="v" :style="{ color: signal.quality >= 4 ? 'var(--g)' : signal.quality >= 3 ? 'var(--y)' : 'var(--r)' }">
					...
					<div class="hist-box">
						...
						<div v-for="(h, i) in histItems" :key="i" class="hist-item">
							...
							<div class="hist-info">
								...
								<div :class="['hist-type', h.type]">{{ h.type.toUpperCase() }}-{{ t('signals.title').includes('Signal') ? 'Signal' : 'Signal' }}</div>
							</div>
					...
					<div class="news-box">
						...
						<div class="news-list">
							<div v-for="n in newsItems" :key="n.headline" class="news-item">
								...
								<div class="news-text"><strong>{{ n.headline }}</strong>{{ n.impact }} <span style="font-size:8px;color:var(--mu2)">· {{ n.age }}</span></div>
							</div>
						</div>
					</div>
				</div>
				...
				<div v-else>
					<div class="bt-box">
						...
						<div v-for="(tr, i) in btTrades" :key="i" class="bt-trade">
							...
							<span class="dates">{{ t('signals.entry') }}: ${{ tr.entry }} → {{ t('signals.entry') }}: ${{ tr.exit }}<br>{{ tr.from }} → {{ tr.to }}</span>
							<span :class="['perf', tr.pnl >= 0 ? 'up' : 'down']">{{ tr.pnl >= 0 ? '+' : '' }}{{ tr.pnl.toFixed(1) }}%</span>
						</div>
					</div>
				</div>
			</div>
...
<style scoped>
.detail-ov { position: fixed; inset: 0; background: rgba(0,0,0,.5); z-index: 500; display: flex; align-items: flex-end; backdrop-filter: blur(4px); }
.detail-page { background: var(--bg); border-radius: 22px 22px 0 0; width: 100%; max-height: 92vh; overflow-y: auto; animation: sheetUp .32s cubic-bezier(.34,1.2,.64,1); }
.detail-hdr { display: flex; align-items: center; gap: 9px; padding: 12px 16px; border-bottom: 1px solid var(--bdr); position: sticky; top: 0; background: rgba(11,14,17,.96); backdrop-filter: blur(16px); z-index: 10; }
.back-btn { width: 32px; height: 32px; border-radius: 8px; background: var(--bg3); border: 1px solid var(--bdr); display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 15px; flex-shrink: 0; color: var(--tx); }
.dh-logo { width: 36px; height: 36px; border-radius: 9px; background: var(--bg3); border: 1px solid var(--bdr); display: flex; align-items: center; justify-content: center; overflow: hidden; flex-shrink: 0; }
.dh-logo img { width: 24px; height: 24px; object-fit: contain; }
.dh-logo-fb { font-size: 10px; font-weight: 800; color: var(--mu); }
.dh-title { flex: 1; }
.dh-ticker { font-size: 19px; font-weight: 800; letter-spacing: -.03em; }
.dh-name { font-size: 10px; color: var(--mu); }
.d-badge { padding: 4px 10px; border-radius: 7px; font-weight: 700; font-size: 9px; }
...
.wl-banner { margin: 12px 14px 0; background: rgba(79,142,247,.07); border: 1px solid rgba(79,142,247,.25); border-radius: 12px; padding: 11px 14px; display: flex; align-items: center; justify-content: space-between; gap: 10px; transition: all .2s; }
.wl-banner-text { font-size: 12px; color: var(--tx); font-weight: 600; }
.wl-banner-sub { font-size: 10px; color: var(--mu); margin-top: 1px; }
.wl-banner-btn { background: var(--b); border: none; border-radius: 9px; padding: 8px 14px; color: #fff; font-weight: 700; font-size: 11px; cursor: pointer; white-space: nowrap; flex-shrink: 0; }
...
.d-tab { flex: 1; padding: 7px; border: none; border-radius: 7px; background: transparent; color: var(--mu); font-weight: 600; font-size: 10px; cursor: pointer; transition: all .2s; }
...
.chart-box { background: var(--bg2); border: 1px solid var(--bdr); border-radius: 13px; padding: 13px; margin-bottom: 13px; }
.cb-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
.cb-title { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .09em; color: var(--mu); }
.period-btns { display: flex; gap: 3px; }
.pb { background: transparent; border: 1px solid var(--bdr); border-radius: 6px; padding: 3px 8px; font-size: 9px; font-weight: 700; color: var(--mu); cursor: pointer; font-family: var(--mono); transition: all .15s; }
.pb.on { background: var(--bg4); border-color: var(--bdr2); color: var(--tx); }
...
.bt-box { background: var(--bg2); border: 1px solid var(--bdr); border-radius: 13px; padding: 13px; margin-bottom: 12px; }
...
.bt-trade { display: flex; align-items: center; gap: 7px; padding: 8px; background: var(--bg); border-radius: 8px; font-size: 10px; border: 1px solid var(--bdr); margin-bottom: 5px; }
