  ...
  <div class="drawer-ov" @click.self="$emit('close')">
    <div class="drawer open">
      ...
      <div class="dr-profile" @click="$emit('close')">
        <div class="dr-profile-top">
          ...
          <div style="flex:1">
            ...
            <div class="dr-prole">Pro-Mitglied</div>
          </div>
        </div>
        ...
        <div style="margin-top:8px;font-size:9px;color:var(--mu);font-family:var(--mono)">Profil bearbeiten →</div>
      </div>
      ...
      <div style="padding:4px 0">
        <div class="dr-settings-label">Einstellungen</div>
        <!-- Theme -->
        <div class="dr-toggle-row" @click="appStore.toggleDarkMode()">
          ...
          <span class="dr-toggle-label">{{ appStore.darkMode ? 'Hell-Modus' : 'Dunkel-Modus' }}</span>
        ...
        <div class="dr-toggle-row" @click="appStore.toggleNotif()">
          ...
          <span class="dr-toggle-label">{{ appStore.notifEnabled ? 'Benachrichtigungen an' : 'Benachrichtigungen aus' }}</span>
          <label class="ios-toggle" @click.stop>
...
const riskMap: Record<string, string> = { conservative: 'Konservativ', moderate: 'Moderat', aggressive: 'Aggressiv', speculative: 'Spekulativ' }
const hzMap: Record<string, string> = { intraday: 'Intraday', short: 'Kurzfristig', medium: 'Mittelfristig', long: 'Langfristig' }

const riskLabel = computed(() => riskMap[authStore.user?.riskProfile || 'moderate'] || 'Moderat')
const horizonLabel = computed(() => hzMap[authStore.user?.horizon || 'medium'] || 'Mittelfristig')
...
.dr-contact { padding: 12px; background: var(--bg3); border-radius: 10px; }
