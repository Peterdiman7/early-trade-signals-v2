	...
	<div class="detail-ov" @click.self="$emit('close')">
		<div class="detail-page">
			...
			<div class="wl-banner"
				...
				<div>
					<div class="wl-banner-text">{{ inWl ? '✓ In deiner Watchlist' : '📌 Zur Watchlist hinzufügen' }}
					</div>
					<div class="wl-banner-sub">{{ inWl ? 'Performance wird verfolgt — klicken zum Entfernen' :
						'Kursveränderung & Haltedauer verfolgen' }}</div>
				</div>
			...
			<div class="detail-body">
				...
				<div class="d-tabs">
					<button :class="['d-tab', { on: dTab === 'overview' }]"
						@click="dTab = 'overview'">Übersicht</button>
					<button :class="['d-tab', { on: dTab === 'analysis' }]" @click="dTab = 'analysis'">Analyse &
						News</button>
					<button :class="['d-tab', { on: dTab === 'backtrack' }]"
						@click="dTab = 'backtrack'">Performance</button>
				</div>

				<div v-if="dTab === 'overview'">
					<div class="info-grid">
						<div class="i-cell"><label>Signal-Datum</label>
							<div class="v">{{ signal.signalDate }}</div>
						</div>
						<div class="i-cell"><label>Signalstärke</label>
							<div class="v"
								:style="{ color: signal.strength === 'Strong' ? 'var(--g)' : signal.strength === 'Moderate' ? 'var(--y)' : 'var(--r)' }">
								{{ signal.strength }}</div>
						</div>
						<div class="i-cell"><label>Qualität</label>
							<div class="v"
								:style="{ color: signal.quality >= 4 ? 'var(--g)' : signal.quality >= 3 ? 'var(--y)' : 'var(--r)' }">
								Q{{ signal.quality }}/5 {{ stars }}</div>
						</div>
						<div class="i-cell"><label>Horizont</label>
							<div class="v"><span :class="['dur-badge', signal.duration]">{{ durLabel }}</span></div>
						</div>
						<div class="i-cell"><label>Sektor</label>
							<div class="v">{{ signal.sector }}</div>
						</div>
						<div class="i-cell"><label>Einstieg</label>
					...
					<div class="chart-box">
						<div class="cb-head">
							<span class="cb-title">Kursverlauf & Signale</span>
				...
				<div v-else-if="dTab === 'analysis'">
					<div class="analysis-box">
						...
						<div class="ab-text">{{ signal.name }} zeigt strukturelle Stärke im Sektor {{ signal.sector }}.
							Signal basiert auf Kombination aus technischer Analyse, Fundamentaldaten und Sentiment.
							Signalstärke: {{ signal.strength }}. Qualität Q{{ signal.quality }}/5 basierend auf
							Momentum-Score und Sektorentwicklung.</div>
					</div>
					<div class="news-box">
						...
						<div class="news-counts">
							<div class="nc-item">
								...
								<div class="nc-lbl">Positiv</div>
							</div>
							<div class="nc-item">
								<div class="nc-num neg">2</div>
								<div class="nc-lbl">Negativ</div>
							</div>
							<div class="nc-item">
								<div class="nc-num neu">2</div>
								<div class="nc-lbl">Neutral</div>
							</div>
						</div>
						<div class="nm-bar">
				...
				<div v-else>
					<div class="bt-box">
						...
						<div class="bt-summary">
							<div class="bts-i"><label>Gesamtrendite</label>
								<div class="v" style="color:var(--g)">+18.4%</div>
							</div>
							<div class="bts-i"><label>Win Rate</label>
								<div class="v">{{ signal.quality * 15 }}%</div>
							</div>
							<div class="bts-i"><label>Trades</label>
								<div class="v">5</div>
							</div>
							<div class="bts-i"><label>Zeitraum</label>
								<div class="v" style="font-size:10px">6 Monate</div>
							</div>
						</div>
						<div style="margin-bottom:11px">
							<ChartPortfolio :totalVal="signal.price" :height="140" />
						</div>
						<div v-for="(t, i) in btTrades" :key="i" class="bt-trade">
							<span :class="['type', t.type]">{{ t.type.toUpperCase() }}</span>
							<span class="dates">Ein: ${{ t.entry }} → Aus: ${{ t.exit }}<br>{{ t.from }} → {{ t.to
								}}</span>
							<span :class="['perf', t.pnl >= 0 ? 'up' : 'down']">{{ t.pnl >= 0 ? '+' : '' }}{{
								t.pnl.toFixed(1) }}%</span>
						</div>
					</div>
				</div>
			</div>
...
const newsItems = [
	...
	{ s: 'pos', headline: 'Analyst erhöht Kursziel', age: '2d', impact: ' — Positiv' },
