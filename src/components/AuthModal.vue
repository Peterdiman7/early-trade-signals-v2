    ...
    <div class="modal">
      ...
      <div v-if="step === 'lang'" class="auth-lang-step">
        <div style="text-align:center;margin-bottom:20px">
          ...
          <p style="font-size:11px;color:var(--mu)">{{ t('auth.chooseLanguage') }}</p>
        </div>
        <div class="auth-lang-grid">
          <button v-for="l in i18n.languages" :key="l.code"
        ...
        <button class="msubmit" style="margin-top:20px" @click="confirmLang">{{ t('auth.continueBtn') }}</button>
      </div>
      ...
      <div v-else>
        <div class="modal-logo">
          ...
          <h2>{{ t('auth.title') }}</h2>
          <p>{{ activeTab === 'register' ? t('auth.registerSubtitle') : t('auth.loginSubtitle') }}</p>
        </div>

        <div class="modal-tabs">
          <button :class="['mtab', { on: activeTab === 'register' }]" @click="activeTab = 'register'">{{ t('auth.register') }}</button>
          <button :class="['mtab', { on: activeTab === 'login' }]" @click="activeTab = 'login'">{{ t('auth.login') }}</button>
        </div>
        ...
        <div v-if="activeTab === 'register'">
          <div class="form-row">
            <div class="fg">
              ...
              <input v-model="form.firstName" type="text" :placeholder="t('auth.firstNamePlaceholder')" />
            </div>
            <div class="fg">
              <label>{{ t('auth.lastName') }}</label>
              <input v-model="form.lastName" type="text" :placeholder="t('auth.lastNamePlaceholder')" />
            </div>
          </div>
          <div class="fg">
        ...
        <div v-else>
          <div class="fg">
            <label>{{ t('auth.email') }}</label>
            <input v-model="form.email" type="email" value="demo@early-trade-signals.com" />
          </div>
          <div class="fg">
            <label>{{ t('auth.password') }}</label>
            <input v-model="form.password" type="password" value="Demo1234!" />
          </div>
...
const selectedLang = ref<Lang>(i18n.locale)
...
function confirmLang() {
  i18n.setLocale(selectedLang.value)
  step.value = 'form'
}

async function doRegister() {
  if (!form.value.firstName) { error.value = t('auth.firstNameRequired'); return }
  error.value = ''; loading.value = true
  try {
    await authStore.register(form.value.firstName, form.value.lastName, form.value.email, form.value.password)
    emit('success')
  } catch { error.value = t('auth.registerFailed') }
...
async function doLogin() {
  ...
  try {
    ...
    emit('success')
  } catch { error.value = t('auth.loginFailed') }
  finally { loading.value = false }
}
</script>
