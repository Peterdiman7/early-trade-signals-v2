		...
		<div class="modal">
			...
			<div v-if="step === 'lang'" class="auth-lang-step">
				...
				<div class="auth-lang-grid">
					<button v-for="l in langs" :key="l.code"
						:class="['auth-lang-card', { active: selectedLang === l.code }]" @click="selectedLang = l.code">
			...
			<div v-else>
				...
				<div class="modal-tabs">
					<button :class="['mtab', { on: activeTab === 'register' }]"
						@click="activeTab = 'register'">Registrieren</button>
					<button :class="['mtab', { on: activeTab === 'login' }]"
						@click="activeTab = 'login'">Einloggen</button>
				</div>

				<!-- Register -->
				<div v-if="activeTab === 'register'">
					<div class="form-row">
						<div class="fg"><label>Vorname</label><input v-model="form.firstName" type="text"
								placeholder="Max" /></div>
						<div class="fg"><label>Nachname</label><input v-model="form.lastName" type="text"
								placeholder="Müller" />
						</div>
					</div>
					<div class="fg"><label>E-Mail</label><input v-model="form.email" type="email"
							placeholder="max@email.com" />
					</div>
					<div class="fg"><label>Passwort</label><input v-model="form.password" type="password"
							placeholder="Mind. 8 Zeichen" /></div>
				...
				<div v-else>
					<div class="fg"><label>E-Mail</label><input v-model="form.email" type="email"
							value="demo@early-trade-signals.com" /></div>
					<div class="fg"><label>Passwort</label><input v-model="form.password" type="password"
							value="Demo1234!" />
					</div>
				...
				<div v-if="error" style="color:var(--r);font-size:12px;text-align:center;margin-top:8px">{{ error }}
				</div>

				<div style="text-align:center;margin-top:12px">
					<button @click="step = 'lang'"
						style="background:none;border:none;color:var(--mu);font-size:10px;cursor:pointer;text-decoration:underline">
						Sprache ändern
					</button>
				</div>
			</div>
