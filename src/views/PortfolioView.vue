    ...
    <ion-content :fullscreen="true">
      <div class="pad">
        <div class="sec-head" style="margin-top:16px">
          <span class="sec-title">💼 Portfolio</span>
        </div>
        <div class="ptf-header">
          <div class="ptf-sub">Gesamtwert (simuliert)</div>
          ...
          <div class="ptf-perf">
            ...
            <div class="ptf-p">
              <label>Unreal. G/V</label>
              <div class="v" :style="{ color: pnl >= 0 ? 'var(--g)' : 'var(--r)' }">{{ pnl >= 0 ? '+' : '' }}${{ pnl.toFixed(2) }}</div>
            </div>
          </div>
        </div>

        <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.09em;color:var(--mu);margin:16px 0 8px">Positionen</div>
        <div v-for="p in positions" :key="p.ticker" class="pos-item" @click="selected = p.sig">
          ...
          <div class="pos-data">
            <div class="pos-d"><label>Anteile</label><div class="v">{{ p.sh }}</div></div>
            <div class="pos-d"><label>Einstand</label><div class="v">${{ p.entry }}</div></div>
            <div class="pos-d"><label>G/V</label><div class="v" :style="{ color: p.pnl >= 0 ? 'var(--g)' : 'var(--r)' }">{{ p.pnl >= 0 ? '+' : '' }}${{ p.pnl.toFixed(0) }}</div></div>
          </div>
        </div>
      </div>
