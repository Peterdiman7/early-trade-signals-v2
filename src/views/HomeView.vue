  ...
  <ion-page>
    ...
    <div class="navbar">
      <div class="nav-top">
        <div class="nav-logo">
          <div class="nldot">
            <svg viewBox="0 0 24 24" fill="none" stroke="#000" stroke-width="2.5" stroke-linecap="round" width="16" height="16">
        ...
        <div style="display:flex;align-items:center;gap:7px">
          <button class="bell-wrap" @click="showNotifSheet = true">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--mu)" stroke-width="2" stroke-linecap="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
    ...
    <ion-content :fullscreen="true">
      ...
      <div class="market-strip">
        <div v-for="m in marketItems" :key="m.label" class="ms-item">
          ...
          <div :class="['ms-val', { 'c-buy': m.up, 'c-sell': !m.up, 'c-hold': m.neutral }]">{{ m.val }}</div>
        </div>
      </div>
      ...
      <div class="pad">
        <div class="sec-head" style="margin-top:16px">
          <span class="sec-title">Top Signale</span>
        ...
        <div class="top-signals-table">
          <div class="ts-col">
            <div class="ts-col-head">
              ...
              <span style="color:var(--g)">Top BUY</span>
            </div>
            <div v-for="s in topBuy" :key="s.id" class="ts-row" @click="openDetail(s)">
              ...
              <div style="flex:1">
                ...
                <div style="font-family:var(--mono);font-size:8px;color:var(--mu)">vor {{ ageLabel(s) }}</div>
              </div>
          ...
          <div class="ts-col">
            <div class="ts-col-head">
              ...
              <span style="color:var(--r)">Top SELL</span>
            </div>
        ...
        <div style="margin-top:14px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:7px">
            <button class="sig-count-btn buy" @click="goSignals('buy')">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>
              <span>Kaufen</span>
              <span class="count-num">{{ sigStore.buySignals.length }}</span>
            </button>
            <button class="sig-count-btn hold" @click="goSignals('hold')">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="10" y1="15" x2="10" y2="9"/><line x1="14" y1="15" x2="14" y2="9"/></svg>
              <span>Halten</span>
              <span class="count-num">{{ sigStore.holdSignals.length }}</span>
            </button>
            <button class="sig-count-btn sell" @click="goSignals('sell')">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><polyline points="23 18 13.5 8.5 8.5 13.5 1 6"/><polyline points="17 18 23 18 23 12"/></svg>
              <span>Verkaufen</span>
      ...
      <div class="pad" style="margin-top:6px;padding-bottom:16px">
        ...
        <div class="acc-block">
          <div class="acc-head" @click="accIndex = !accIndex">
            <span class="acc-title">Index-Performance % (6M)</span>
            <span class="acc-chg" style="color:var(--g)">S&P +8.2% · NQ +12.1%</span>
...
const today = computed(() => new Date().toLocaleDateString('de-DE', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' }))
...
const subText = computed(() => {
  const today = sigStore.signals.filter(s => s.signalDate === '2026-02-22').length
  return `${today} neue Signale heute · ${sigStore.buySignals.length} Kaufen · ${sigStore.signals.filter(s => s.quality >= 4).length}× Q4+`
})
...
const marketItems = [
  ...
  { label: 'VIX', val: '18.4', up: false, neutral: true },
  { label: 'Sentiment', val: 'Bullish', up: true }
]
...
/* Navbar */
.navbar {
  background: rgba(11,14,17,.94); border-bottom: 1px solid var(--bdr);
  padding: 0 16px; position: sticky; top: 0; z-index: 100; backdrop-filter: blur(20px);
}
...
/* Signal count buttons */
.sig-count-btn {
  border-radius: 11px; padding: 13px 6px; font-weight: 700; font-size: 11px; cursor: pointer;
